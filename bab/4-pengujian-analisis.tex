...
Sesuai dengan hasil yang ada pada Tabel \ref{tb:HasilValDefault}, yang mana merupakan hasil metrik dari model seluruh arsitektur ResNet dengan nilai validasi terbaik, arsitektur \emph{ResNet-101} dan \emph{ResNet-34} mendapatkan nilai akurasi yang sama, yaitu sebesar \textbf{0.8049}.
...
\subsection{GRAD-CAM}
\label{sec:43}
Bagian ini memaparkan hasil pengujian menggunakan Grad-CAM, yang menghasilkan \emph{heatmap} yang ditumpangkan pada gambar asli. \emph{heatmap} mengungkapkan wilayah atau karakteristik tertentu dalam gambar yang dianggap penting untuk proses pengambilan keputusan model. Bagian ini akan menganalisis hasil pengujian menggunakan subset data pelatihan untuk mengidentifikasi area yang disorot yang dianggap penting atau berbeda berdasarkan model yang dilatih. 
...
Model yang dipilih untuk contoh hasil analisis GRAD-CAM adalah \emph{ResNet-18 default}, \emph{ResNet-18 class-weighted}, \emph{ResNet-152 class-weighted}. Pemilihan ini dikarenakan model - model ini memiliki akurasi klasifikasi \emph{class} PDR yang tinggi, tanpa mengorbankan banyak dari akurasi keseluruhan. Model ini juga memiliki nilai Kappa yang cukup tinggi yaitu sekitar 0,74. Model ResNet-18 dipilih dikarenakan memiliki nilai Kappa tertinggi, namun akurasi klasifikasi untuk \emph{class} PDR cukup rendah dibanding ResNet-152, sedangkan akurasi keseluruhan dari ResNet-18 sedikit lebih unggul dari ResNet-152 yaitu sebesar 0,0081.
...
Bagian gambar yang disoroti dengan warna 'hangat' yaitu kuning, oranye, dan merah merupakan bagian yang dianggap penting oleh model dalam pengambilan Keputusan. %Representasi fitur objek gambar lebih tersebar luas karena objek di dalam \emph{frame} adalah pembuluh darah yang memanjang dan bercabang, dibandingkan dengan entitas tunggal seperti manusia atau hewan di dalam \emph{frame} gambar.

\subsection*{Kelas (0) (Non-DR)}
\begin{figure}[H]
	\centering
	\subfloat[\centering Gambar Asli]{{\includegraphics[width=4cm]{gambar/class0_0.png} }}%
	\subfloat[\centering Gambar \emph{heatmap} dari ResNet-18 Default Weight]{{\includegraphics[width=4cm]{gambar/0_0_18def.png} }}%
	\subfloat[\centering Gambar \emph{heatmap} dari ResNet-18 Class-weighted]{{\includegraphics[width=4cm]{gambar/0_0_18class.png} }}%
...
\begin{itemize}
	\item \textbf{ResNet 18 (Standar)}: Aktivasi signifikan di sekitar area perifer retina, dengan fokus minimal pada area makula pusat.
	\item \textbf{ResNet 18 (\emph{Class-weighted})}: Fokus yang lebih seimbang dengan peningkatan perhatian pada area perifer, menunjukkan area ini penting dalam pengambilan keputusan model.
	\item \textbf{ResNet 152 (\emph{Class-weighted})}: Fokus yang sangat terperinci dan terkonsentrasi pada area perifer retina, menunjukkan kapasitas model yang lebih tinggi untuk menangkap dan menekankan fitur-fitur penting di area ini. Area pusat tidak dianggap penting di semua model.
\end{itemize}
%    \item \textbf{ResNet-18 (Standar)}: Aktivasi signifikan di sekitar diskus optikus dan daerah perifer, dengan fokus yang luas di seluruh retina.
%    \item \textbf{ResNet-18 (Penyesuaian Bobot Kelas)}: Fokus yang lebih seimbang dengan peningkatan perhatian pada diskus optikus dan daerah perifer, menekankan fitur struktural.
%    \item \textbf{ResNet-152 (Penyesuaian Bobot Kelas)}: Fokus yang sangat terperinci dan terkonsentrasi pada diskus optikus dan retina perifer, menunjukkan kapasitas model yang lebih tinggi dan efektivitas penyesuaian bobot kelas.

\subsection*{Kelas (1) (NPDR)}
\begin{figure}[H]
	\centering
...
\begin{itemize}
	\item \textbf{ResNet 18 (Standar)}: Aktivasi signifikan di sekitar diskus optik dan area perifer, dengan fokus luas di seluruh retina. Area makula pusat menunjukkan beberapa aktivasi tetapi kurang penting.
	\item \textbf{ResNet 18 (\emph{Class-weighted})}: Fokus yang lebih seimbang dengan peningkatan perhatian pada diskus optik dan area perifer, menekankan fitur-fitur struktural. Area pusat tetap kurang signifikan.
	\item \textbf{ResNet 152 (\emph{Class-weighted})}: Fokus yang sangat terperinci dan terkonsentrasi pada diskus optik dan retina perifer, dengan beberapa perhatian pada area perifer bawah yang spesifik. Ini menunjukkan kapasitas model yang lebih tinggi dan efektivitas penyesuaian bobot kelas dalam menangkap fitur-fitur relevan untuk klasifikasi NPDR.
\end{itemize}

\subsection*{Kelas (2) (PDR)}
\begin{figure}[H]
	\centering
...
\begin{itemize}
	\item \textbf{ResNet 18 (Standar)}: Aktivasi signifikan di sekitar diskus optik dan area perifer, dengan fokus luas di seluruh retina. Area makula pusat menunjukkan beberapa aktivasi tetapi kurang penting.
	\item \textbf{ResNet 18 (\emph{Class-weighted})}: Fokus yang lebih seimbang dengan peningkatan perhatian pada diskus optik dan area perifer, menekankan fitur-fitur struktural. Area pusat tetap kurang signifikan.
	\item \textbf{ResNet 152 (\emph{Class-weighted})}: Fokus yang sangat terperinci dan terkonsentrasi pada diskus optik dan retina perifer, dengan beberapa perhatian pada area perifer bawah yang spesifik. Ini menunjukkan kapasitas model yang lebih tinggi dan efektivitas penyesuaian bobot kelas dalam menangkap fitur-fitur relevan untuk klasifikasi PDR.
\end{itemize}
